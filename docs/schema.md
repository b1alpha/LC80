...
**Optional:** `category`, `status`, `cost_cad` (all-in estimate), `parts_cad` (parts share of `cost_cad`; labour is shown as the difference), `parts[]` (checklist of `{item, have: true|false, note}` — renders Have/Need labels inside the row's Notes), `time`, `notes`, `on_hand`, `still_needed`, `checked` (default: false), `id` (slug)
